...
This model work well if the average user doesn't rate a particularly good/popular movie with a large positive $b_{i}$, and vice versa.

We can affirm to have built a machine learning algorithm to predict movie ratings using the MovieLens dataset.
The regularized model including the effect of user is characterized by the lower RMSE value and is hence the optimal model to use for the present project. With this model, we have achieved our goal of creating an algorith with RMSE(0.8648170) lower than 0.8775.
